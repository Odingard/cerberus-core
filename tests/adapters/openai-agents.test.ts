...
describe('createCerberusGuardrail', () => {
  ...
  it('should forward the coverage report (clean config has no unwrapped declarations)', () => {
    const guardrail = createCerberusGuardrail(makeConfig());
    expect(guardrail.coverage).toBeDefined();
    expect(guardrail.coverage.hasUnwrappedDeclarations).toBe(false);
    expect(guardrail.coverage.tools.map((t) => t.toolName).sort()).toEqual([
  ...
  it('should surface a declared-but-unwrapped tool on coverage (the adapter parity case)', () => {
    ...
    expect(guardrail.coverage.hasUnwrappedDeclarations).toBe(true);
    expect(guardrail.coverage.declaredButUnwrapped.map((d) => d.toolName)).toContain('sendEmial');
    warn.mockRestore();
    guardrail.destroy();
  });
});
