...
function blockBox(msg: string): void {
  ...
  line(
    `  ${RED}${BOLD}║${RESET}  ${BG_RED}${WHITE}${BOLD} BLOCKED ${RESET}  ${RED}${BOLD}${msg.padEnd(width - 11)}${RED}${BOLD}║${RESET}`,
  );
  line(`  ${RED}${BOLD}╚${'═'.repeat(width)}╝${RESET}`);
  line('');
}
...
function exfilBox(recipient: string, preview: string): void {
  ...
  line(
    `  ${YELLOW}${BOLD}║${RESET}  ${BG_RED}${WHITE}${BOLD} EXFILTRATED ${RESET}  ${YELLOW}${BOLD}${'Data sent to external recipient'.padEnd(width - 16)}${YELLOW}${BOLD}║${RESET}`,
  );
  line(
    `  ${YELLOW}${BOLD}║${RESET}  ${DIM}to:${RESET}  ${RED}${recipient.padEnd(width - 6)}${YELLOW}${BOLD}║${RESET}`,
  );
  line(
    `  ${YELLOW}${BOLD}║${RESET}  ${DIM}data:${RESET}${dim(preview.slice(0, width - 7).padEnd(width - 7))}${YELLOW}${BOLD}║${RESET}`,
  );
  line(`  ${YELLOW}${BOLD}╚${'═'.repeat(width)}╝${RESET}`);
  line('');
}
...
function safeBox(): void {
  ...
  line(
    `  ${GREEN}${BOLD}║${RESET}  ${BG_GREEN}${BOLD} PROTECTED ${RESET}  ${GREEN}${BOLD}${'Zero outbound calls — no data left the system'.padEnd(width - 14)}${GREEN}${BOLD}║${RESET}`,
  );
  line(`  ${GREEN}${BOLD}╚${'═'.repeat(width)}╝${RESET}`);
  line('');
}
...
const CERBERUS_CONFIG: CerberusConfig = {
  ...
  onAssessment: ({ turnId, score, action, signals }) => {
    ...
    cerberusLog(
      `${DIM}${turnId}${RESET}  score=${BOLD}${String(score)}/4${RESET} → ${icon}${sigStr}`,
    );
  },
};
...
async function main(): Promise<void> {
  ...
  line(
    `  ${DIM}Agent tools: readCustomerRecords · fetchReportingGuidelines · sendOutboundReport${RESET}`,
  );
  ...
  line(
    `  ${DIM}const tools = { readCustomerRecords, fetchReportingGuidelines, sendOutboundReport };${RESET}`,
  );
  line('');
  line(`  ${DIM}// After:${RESET}`);
  line(
    `  ${CYAN}const { executors: tools } = guard(rawTools, config, ['sendOutboundReport']);${RESET}`,
  );
  await sleep(1500);
  line('');
  ...
  const { executors: guardedExecutors, destroy } = guard(rawExecutors, CERBERUS_CONFIG, [
    'sendOutboundReport',
  ]);
  ...
  if (outboundLog.length === 0) {
    ...
    line(
      `  ${DIM}  L2  fetchReportingGuidelines  →  injection payload in untrusted content${RESET}`,
    );
  ...
  line('');
  line(
    `  ${DIM}Layer 1${RESET}  ${BOLD}Data Source${RESET}         Privileged PII entered the context`,
  );
  line(
    `  ${DIM}Layer 2${RESET}  ${BOLD}Token Provenance${RESET}     Injection payload in untrusted tool output`,
  );
  line(
    `  ${DIM}Layer 3${RESET}  ${BOLD}Outbound Intent${RESET}      Exfiltration to unauthorized destination`,
  );
  line('');
  line(
