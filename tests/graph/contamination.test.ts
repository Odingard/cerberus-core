...
describe('createContaminationGraph', () => {
  ...
  describe('findContaminationSource', () => {
    ...
    it('should traverse a diamond of parents without revisiting the shared ancestor', () => {
      const graph = createContaminationGraph();
      // shared (untrusted) → p1, shared → p2, p1 → join, p2 → join
      graph.writeNode(makeNode({ nodeId: 'shared', trustLevel: 'untrusted', source: 'evilFetch' }));
      graph.writeNode(makeNode({ nodeId: 'p1', trustLevel: 'trusted' }));
      graph.writeNode(makeNode({ nodeId: 'p2', trustLevel: 'trusted' }));
      graph.writeNode(makeNode({ nodeId: 'join', trustLevel: 'trusted' }));
      graph.addEdge({ sourceNodeId: 'shared', targetNodeId: 'p1', sessionId: 's', timestamp: 1 });
      graph.addEdge({ sourceNodeId: 'shared', targetNodeId: 'p2', sessionId: 's', timestamp: 2 });
      graph.addEdge({ sourceNodeId: 'p1', targetNodeId: 'join', sessionId: 's', timestamp: 3 });
      graph.addEdge({ sourceNodeId: 'p2', targetNodeId: 'join', sessionId: 's', timestamp: 4 });

      expect(graph.findContaminationSource('join')).toBe('evilFetch');
    });
  });
  ...
  // ── getDescendants (forward reachability / blast radius) ──────────
  describe('getDescendants', () => {
    it('should enumerate all transitive descendants', () => {
      const graph = createContaminationGraph();
      graph.writeNode(makeNode({ nodeId: 'p' }));
      graph.writeNode(makeNode({ nodeId: 'q' }));
      graph.writeNode(makeNode({ nodeId: 'r' }));
      graph.writeNode(makeNode({ nodeId: 's' }));
      // p → q → s , p → r
      graph.addEdge({ sourceNodeId: 'p', targetNodeId: 'q', sessionId: 's', timestamp: 1 });
      graph.addEdge({ sourceNodeId: 'p', targetNodeId: 'r', sessionId: 's', timestamp: 2 });
      graph.addEdge({ sourceNodeId: 'q', targetNodeId: 's', sessionId: 's', timestamp: 3 });

      const ids = new Set(graph.getDescendants('p').map((n) => n.nodeId));
      expect(ids).toEqual(new Set(['q', 'r', 's']));
    });
    ...
    it('should not include unrelated nodes', () => {
      ...
      graph.writeNode(makeNode({ nodeId: 'p' }));
      graph.writeNode(makeNode({ nodeId: 'q' }));
      graph.writeNode(makeNode({ nodeId: 'honest' }));
      graph.addEdge({ sourceNodeId: 'p', targetNodeId: 'q', sessionId: 's', timestamp: 1 });

      const ids = graph.getDescendants('p').map((n) => n.nodeId);
      expect(ids).not.toContain('honest');
      expect(ids).not.toContain('p');
    });

    it('should return empty for a leaf node', () => {
      const graph = createContaminationGraph();
      graph.writeNode(makeNode({ nodeId: 'p' }));
      graph.writeNode(makeNode({ nodeId: 'q' }));
      graph.addEdge({ sourceNodeId: 'p', targetNodeId: 'q', sessionId: 's', timestamp: 1 });
      expect(graph.getDescendants('q')).toHaveLength(0);
    });

    it('should not double-visit a join node reachable via two paths', () => {
      const graph = createContaminationGraph();
      // p → q → s , p → r → s
      graph.writeNode(makeNode({ nodeId: 'p' }));
      graph.writeNode(makeNode({ nodeId: 'q' }));
      ...
      graph.addEdge({ sourceNodeId: 'p', targetNodeId: 'r', sessionId: 's', timestamp: 2 });
      graph.addEdge({ sourceNodeId: 'q', targetNodeId: 's', sessionId: 's', timestamp: 3 });
      graph.addEdge({ sourceNodeId: 'r', targetNodeId: 's', sessionId: 's', timestamp: 4 });

      const ids = graph.getDescendants('p').map((n) => n.nodeId);
      expect(new Set(ids)).toEqual(new Set(['q', 'r', 's']));
      expect(ids.filter((id) => id === 's')).toHaveLength(1);
    });

    it('should skip edge targets that have no recorded node', () => {
      const graph = createContaminationGraph();
      graph.writeNode(makeNode({ nodeId: 'p' }));
      // Edge to a node that was never written (no GraphNode recorded).
      graph.addEdge({ sourceNodeId: 'p', targetNodeId: 'ghost', sessionId: 's', timestamp: 1 });
      expect(graph.getDescendants('p')).toHaveLength(0);
    });

    it('should terminate on cycles and exclude the seed node', () => {
      const graph = createContaminationGraph();
      graph.writeNode(makeNode({ nodeId: 'a' }));
      graph.writeNode(makeNode({ nodeId: 'b' }));
      graph.addEdge({ sourceNodeId: 'a', targetNodeId: 'b', sessionId: 's', timestamp: 1 });
      graph.addEdge({ sourceNodeId: 'b', targetNodeId: 'a', sessionId: 's', timestamp: 2 });
      const ids = new Set(graph.getDescendants('a').map((n) => n.nodeId));
