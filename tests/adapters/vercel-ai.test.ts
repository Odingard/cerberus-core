...
describe('guardVercelAI', () => {
  ...
  it('should forward the coverage report (clean config has no unwrapped declarations)', () => {
    ...
    expect(result.coverage.tools.map((t) => t.toolName).sort()).toEqual([
      'fetchExternalContent',
      'readPrivateData',
      'sendOutboundReport',
    ]);
  ...
  it('should surface a declared-but-unwrapped tool on coverage (the adapter parity case)', () => {
    ...
    const result = guardVercelAI(makeTools(), {
      cerberus: CONFIG,
      outboundTools: ['sendEmial'],
    });
    expect(result.coverage.hasUnwrappedDeclarations).toBe(true);
    expect(result.coverage.declaredButUnwrapped.map((d) => d.toolName)).toContain('sendEmial');
    warn.mockRestore();
  });
});
