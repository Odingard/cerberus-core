...
import { computeCoverageCommitment } from '../engine/coverage.js';
import { deriveEnforcementPosture } from '../graph/enforcement-posture.js';
...
export function guardMultiAgent(
  ...
  // Bind the coverage report in force into the signed manifest, so the receipt
  // attests *what was protected* (the per-layer coverage posture), not just the
  // authorized capability surface.
  const coverageCommitment = computeCoverageCommitment(baseResult.coverage);

  // Bind the effective per-layer enforcement posture in force (Axis 3a), so the
  // receipt also attests *what enforcement authority was in force* — a tampered
  // posture fails integrity.
  const enforcementPosture = deriveEnforcementPosture(config);

  const delegationGraph = createDelegationGraph(
    session.sessionId,
    {
      agentId: rootAgentId,
      agentType,
  ...
  const spawnAgent = (
    ...
    // Verify graph integrity before mutation, honouring the same verifier
    // override the per-turn gate uses (the public-key-only / KMS path).
    if (!verifyGraphIntegrity(delegationGraph, config.manifestVerifier)) {
