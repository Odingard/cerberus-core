...
import { createDelegationGraph, type DelegationGraph } from '../../src/graph/delegation.js';
...
describe('verifyManifestBeforeTurn', () => {
  ...
  it('returns KEY_ID_MISMATCH when an explicit verifier has a different key', () => {
    ...
    const result = verifyManifestBeforeTurn(graph, 'session-1', 'turn-002', otherVerifier);
  ...
  it('returns ALGORITHM_MISMATCH when verifier uses a different algorithm', () => {
    ...
    const result = verifyManifestBeforeTurn(graph, 'session-1', 'turn-003', hmacVerifier);
