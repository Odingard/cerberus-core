...
export interface VercelAIGuardResult {
  ...
  /**
   * Reset session state and assessments (graph/ledger persist; sessionId
   * rotates). `carriedReads` (optional) is the cross-session carry-channel
   * hand-off (Spec B): node IDs carried across the reset that seed the rotated
   * scope's observed reads so post-reset writes anchor the carried deps.
   */
  ...
  /**
   * Tool coverage report for the underlying guard() invocation: per-tool
   * L1/L3/L4 coverage, tools declared in config but never wrapped (their
   * protection silently never runs), and wrapped tools left at default
   * `unknown` trust. Assert on `coverage.hasUnwrappedDeclarations` in
   * deployment checks.
   */
