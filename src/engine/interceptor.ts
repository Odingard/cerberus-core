...
import { extractNodeId, defaultExtractContent } from '../layers/l4-memory.js';
import type { MemoryToolConfig } from '../layers/l4-memory.js';
import type { MemoryTraceRecorder } from '../middleware/trace-capture.js';
...
// runtime-hooks.ts for the dependency-inversion seam.
import { emitTelemetry, emitEnforcement } from './runtime-hooks.js';
...
export function interceptToolCall(
  ...
  const finalizeAssessment = (
    ...
    // Enforcement gateway dispatch — fire-and-forget on interrupt decisions.
    // The licensed dispatcher (injected by @cerberus-ai/enterprise) resolves
    // gateways, builds the signal, and dispatches; open default is a no-op.
    if (outcome.blocked && config.enforcement) {
      emitEnforcement({
        sessionId: session.sessionId,
        assessment,
        outcome,
  ...
  return async (args: Record<string, unknown>): Promise<string> => {
    ...
    // Trace capture (opt-in) — record the replayable memory op for this call.
    // Independent of L4 detection (works even when memoryTracking is off) and
    // zero-cost when no recorder is configured (a single falsy check). Write
    // content is redacted by the recorder; raw content is never stored.
    if (recorder && memoryTools && memoryTools.length > 0) {
      const captureConfig = memoryTools.find((t) => t.toolName === toolName);
      if (captureConfig) {
        const captureNodeId = extractNodeId(args, captureConfig);
        if (captureNodeId !== undefined) {
          if (captureConfig.operation === 'read') {
            recorder.recordRead(captureNodeId);
          } else {
            const extractContent = captureConfig.extractContent ?? defaultExtractContent;
            recorder.recordWrite(captureNodeId, extractContent(args, result));
          }
        }
      }
    }
