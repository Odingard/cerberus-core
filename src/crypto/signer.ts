 ...
 *
 * Two adapters ship with the open core:
 *
...
export class HmacSigner implements SignerVerifier {
  ...
  constructor(opts: HmacSignerOptions = {}) {
    ...
    this.keyId = opts.keyId ?? createHash('sha256').update(this.#key).digest('hex').slice(0, 16);
  }
...
export class Ed25519Signer implements SignerVerifier {
  ...
  sign(payload: string): string {
    return cryptoSign(null, Buffer.from(payload), this.#privateKey).toString('hex');
  }
